...
def unsaved_note(m: dict) -> str:
    ...
    # У стикера из пользовательского набора эмодзи может не быть вовсе. Тогда
    # пометка всё равно не «переспроси»: стикер нельзя прислать иначе, и просьба
    # переслать его другим способом выглядит глупо. Говорим, что это стикер.
    sticker = m.get("sticker")
    if isinstance(sticker, dict):
        подпись = sticker.get("emoji") or sticker.get("set_name") or ""
        return f"[стикер {подпись}]".replace(" ]", "]")
