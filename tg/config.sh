...
tg_api() {
  ...
  # Идемпотентен ли метод — от этого зависит, можно ли повторять при таймауте.
  # Повтор setMessageReaction ставит ту же реакцию, повтор sendMessage создаёт
  # ВТОРОЕ сообщение. Список белый, а не чёрный: незнакомый метод считается
  # неидемпотентным, и худшее, что будет, — отсутствие повтора.
  local idempotent=0
  case "$method" in
    setMessageReaction|editMessageText|editForumTopic|pinChatMessage|\
    unpinChatMessage|deleteMessage|getMe|getChat|getChatMember|\
    getForumTopicIconStickers|getFile) idempotent=1 ;;
  esac

  # «|| rc=$?» обязателен, а не для красоты. Скрипты идут с set -e, и голое
  ...
  local out rc=0 attempt=1 max=3
  while : ; do
    rc=0
    ...
    # Три разных случая, и повторять можно не во всех:
    #   6, 7 — имя не разрешилось / не удалось соединиться. Запрос НЕ УШЁЛ,
    #          повтор безопасен для любого метода.
    #   28   — таймаут. Запрос мог дойти, а потеряться мог ответ. Повторять
    #          можно только идемпотентное, иначе владелец получит дубль.
    #   иное — считаем неопределённым, ведём себя как при таймауте.
    case "$rc" in
      6|7) : ;;
      *)   [ $idempotent -eq 1 ] || break ;;
    esac
